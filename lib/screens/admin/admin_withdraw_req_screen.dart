...
/// admin sets ONE base rate per game (ads required → currency given,
...
class _AdminWithdrawReqScreenState extends State<AdminWithdrawReqScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final rates =
          await AdminService.instance.getWithdrawRequirements();
  ...
  Future<void> _openModal({
    ...
    String? selectedGameId =
        existing?['game_id'] as String? ??
        _games.first['id'] as String;

    final adsController = TextEditingController(
      text: existing?['ads_required']?.toString() ?? '',
    );

    final givenController = TextEditingController(
      text: existing?['currency_given']?.toString() ?? '',
    );

    final targetController = TextEditingController(
      text: existing?['target_currency']?.toString() ?? '',
    );
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  _numField(
                    targetController,
                    'Target — Max Currency (e.g. 1000)',
                    onChanged: () =>
                        setDialogState(() {}),
                  ),

                  if (adsController.text.isNotEmpty &&
                      ...
                      child: GlassCard(
                        ...
                        child: Builder(
                          builder: (context) {
                            ...
                            final totalAdsNeeded =
                                schedule.last['ads'];

                            return Text(
                              'Every $ads ad${ads == 1 ? '' : 's'} → earn ${given.toStringAsFixed(2)}. '
                              'Total cycles: ${schedule.length} · '
                              'Total ads needed: $totalAdsNeeded. '
                        ...
                        child: ElevatedButton(
                          onPressed: () {
                            ...
                            if (given >= target) {
                              setDialogState(
                                () => errorText =
                                    'Target must be greater than currency per cycle.',
                              );
                              return;
                            }
    ...
    if (saved != true || selectedGameId == null) {
      return;
    }

    try {
      await AdminService.instance
          .saveWithdrawRequirement(
        id: existing?['id'] as String?,
        gameId: selectedGameId!,
        adsRequired:
            int.parse(adsController.text),
        currencyGiven:
            double.parse(givenController.text),
        targetCurrency:
                                    ...
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          Text(
                                            'Every $ads ads = ${given.toStringAsFixed(2)} · '
